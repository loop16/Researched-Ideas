...
class ModelAdaptersV3:
    ...
    def __init__(self, data_dir: str, max_date: Optional[date] = None):
        ...
        self.max_date = max_date  # Maximum date to use from historical databases
    ...
    def _filter_by_max_date(self, df: pd.DataFrame) -> pd.DataFrame:
        """Filter DataFrame by max_date if set, using day_date, day_id, or _day_date columns.
        
        This is optimized to avoid unnecessary work when max_date is None.
        """
        if df is None or df.empty or self.max_date is None:
            return df
        df = df.copy()
        day_series = None
        # Try different date column names - check _day_date first (already converted)
        if "_day_date" in df.columns:
            day_series = df["_day_date"]
        elif "day_date" in df.columns:
            day_series = pd.to_datetime(df["day_date"], errors="coerce").dt.date
        elif "day_id" in df.columns:
            day_series = pd.to_datetime(df["day_id"], errors="coerce").dt.date
        if day_series is not None:
            df = df[day_series < self.max_date]
        return df

    def _cutoff_date(self, ny_day: Optional[date]) -> Optional[date]:
        """Return ny_day minus two days for leakage protection."""
        if ny_day is None:
            return None
        try:
            return ny_day - timedelta(days=2)
        except Exception:
            return None

    def _apply_cutoff(self, df: pd.DataFrame, cutoff: Optional[date], columns: Optional[list[str]] = None) -> pd.DataFrame:
        """Filter a dataframe to rows strictly before cutoff using the first matching date-like column."""
        if df is None or df.empty or cutoff is None:
            return df
        cols = columns or ["_day_date", "day_date", "day_id", "pair_day", "week_id"]
        for col in cols:
    ...
    def _modeldb_row_for_day(self, symbol: Optional[str], ny_day: Any, cutoff: Optional[date] = None) -> Optional[pd.Series]:
        ...
        if isinstance(ny_day, datetime):
            ...
            if pd.isna(ts):
                return None
            target_day = ts.date()
        if cutoff is not None and target_day >= cutoff:
            return None
    ...
    def _prior_wddrb_row(self, symbol: str, ny_day: date, cutoff: Optional[date] = None) -> Optional[pd.Series]:
        history = self._get_wddrb_for_symbol(symbol)
        if history.empty or "_day_date" not in history.columns:
            return None
        if cutoff is not None and ny_day >= cutoff:
            return None
        mask = history["_day_date"] == ny_day
        if mask.any():
            idx = history.loc[mask].index.max()
            return history.loc[idx]
        prior_mask = history["_day_date"] < ny_day
        if cutoff is not None:
            prior_mask &= history["_day_date"] < cutoff
    ...
    def _wddrb_row_for_day(self, symbol: str, ny_day: Optional[date], cutoff: Optional[date] = None) -> Optional[pd.Series]:
        if ny_day is None or (cutoff is not None and ny_day >= cutoff):
            return None
        history = self._get_wddrb_for_symbol(symbol)
        if history.empty or "_day_date" not in history.columns:
            return None
    ...
    def _range_size_bucket(self, symbol: str, anchor_day: Optional[date], range_pct: Optional[float]) -> str:
        ...
        if df.empty:
            return "unknown"
        hist = df.copy()
        cutoff = self._cutoff_date(anchor_day)
        if anchor_day is not None:
            ...
            if day_col is not None:
                hist = hist[day_col.dt.date < (cutoff or anchor_day)]
    ...
    def _box_history_filtered(self, df: pd.DataFrame, anchor_day: Optional[date], filters: List[Tuple[str, Any]], min_rows: int = 25) -> pd.DataFrame:
        ...
        if anchor_day is not None:
            ...
            if day_series is not None:
                hist = hist[day_series.dt.date < (cutoff or anchor_day)]
        if hist.empty:
            hist = df.copy()
    ...
    def _filter_modeldb_context(self, bar: Bar, out: Dict, target: str, prior_prefix: str) -> pd.DataFrame:
        ...
        cutoff = self._cutoff_date(self.engine._ny_date_from_epoch(bar.ts))
        df = self._apply_cutoff(df, cutoff, ["_day_date", "day_date", "day_id"])
        # Day filter
    ...
    def get_range_session_metrics_v3(self, bar: Bar) -> RangeSessionOut:
        ...
        ny_day = self.engine._ny_date_from_epoch(bar.ts)
        cutoff = self._cutoff_date(ny_day)
        ...
        try:
            ...
            df_all = self._apply_cutoff(
                self._get_metrics_for_symbol(bar.symbol),
                cutoff,
                ["_day_date", "day_date", "day_id", "Day_Date", "Date"],
            )
            if not df_all.empty:
    ...
    def get_box_daily_context_v3(self, bar: Bar) -> BoxDailyOut:
        ...
        try:
            ...
            bar_dt = datetime.fromtimestamp(bar.ts)
        anchor_day = self.engine._asia_day_anchor(bar_dt, bar.symbol) if hasattr(self.engine, "_asia_day_anchor") else ny_day
    ...
    def get_seq_intraday_context_v3(self, bar: Bar) -> SeqIntradayOut:
        ...
        ny_day = self.engine._ny_date_from_epoch(bar.ts)
        cutoff = self._cutoff_date(ny_day)
        seq_hist = self._apply_cutoff(seq_hist, cutoff, ["pair_day", "day_id", "day_date", "_day_date"])
    ...
    def get_wddrb_context(self, bar: Bar) -> WddrbOut:
        ...
        cutoff = self._cutoff_date(ny_day_date)
        history = self._apply_cutoff(history, cutoff, ["_day_date", "day_date", "day_id"])
        active_day = self._wddrb_active_day(bar.ts, ny_day_date)
        cache_day = active_day or ny_day_date
        modeldb_row = self._modeldb_row_for_day(symbol, cache_day, cutoff)
        current_row = self._wddrb_row_for_day(symbol, cache_day, cutoff)
        prev_day = prev_business_day(cache_day) if cache_day else None
        prev_row = self._prior_wddrb_row(symbol, prev_day, cutoff) if prev_day else None
    ...
    def get_wddrb_model_context(self, bar: Bar) -> WddrbModelOut:
        ...
        ref_row = self._wddrb_row_for_day(symbol, reference_day, cutoff)
        if ref_row is None:
            ref_row = self._prior_wddrb_row(symbol, reference_day, cutoff)
            if ref_row is None:
                return WddrbModelOut(stage="waiting_reference", notes=["no reference history"])
            notes.append("fallback_reference_day")
        ...
        if phase == "preopen":
            ...
            if bucket is None:
                bucket = self._normalize_wddrb_value(ref_row.get("open_bucket_quarters"))
                if bucket:
                    notes.append("fallback_open_bucket_from_history")
            if bucket is None:
    ...
    def _since_asia_start_bucket(self, ts_epoch: int, bucket_minutes: int = 30, anchor_day: Optional[date] = None) -> int:
        ...
        if anchor_day is None:
            bar_date = dt.date()
            bar_time = dt.time()
            anchor_day = bar_date if bar_time >= time(20, 30) else bar_date - timedelta(days=1)
    ...
    def get_seq_asia_context_v3(self, bar: Bar) -> SeqAsiaOut:
        ...
        bar_dt = datetime.fromtimestamp(bar.ts, tz=ZoneInfo(NY_TZ))
        if hasattr(self.engine, "_asia_day_anchor"):
            anchor_day = self.engine._asia_day_anchor(bar_dt, bar.symbol)
        else:
        ...
        t_bucket_30m = self._since_asia_start_bucket(bar.ts, anchor_day=anchor_day)
        ...
        if "day_id" in seq_hist.columns:
            ...
            seq_hist = seq_hist[day_col < (cutoff or anchor_day)]
        seq_hist = self._apply_cutoff(seq_hist, cutoff, ["day_id", "day_date", "_day_date"])
        if seq_hist.empty:
            seq_hist = seq_hist_raw.copy()
    ...
    def get_seq_daycycle_context_v3(self, bar: Bar) -> SeqDaycycleOut:
        ...
        cutoff = self._cutoff_date(anchor_day)
        seq_hist_symbol = self._apply_cutoff(seq_hist_symbol, cutoff, ["day_id", "day_date", "_day_date"])
        ...
        if not hist_slice.empty and "day_id" in hist_slice.columns:
            ...
            prior = hist_slice[hist_slice["day_id"] < pd.Timestamp(cutoff or anchor_day)]
    ...
    def get_first_range_candle_context_v3(self, bar: Bar) -> FirstRangeCandleOut:
        ...
        cutoff = self._cutoff_date(self.engine._ny_date_from_epoch(bar.ts))
        df = self._apply_cutoff(df, cutoff, ["day_date", "_day_date", "day_id"])
    ...
    def get_weekly_context_v3(self, bar: Bar) -> WeeklyOut:
        ...
        cutoff = self._cutoff_date(week_start)

        hist_slice = df
        hist_rows = len(hist_slice)
        if "week_id" in df.columns:
            week_ids = pd.to_datetime(df["week_id"], errors="coerce")
            mask = week_ids.dt.date < (cutoff or week_start)
            filtered = df[mask]
            if not filtered.empty:
                hist_slice = filtered
                hist_rows = len(hist_slice)
